...
import {performanceTechLevelList} from "./performanceTechLevelList";

describe('Performance Tech Level List', () => {
    test('tests explorer tech level list', () => {
        const totalProbability = performanceTechLevelList.reduce((sum, level) => sum + level[1], 0);
        expect(totalProbability).toBeCloseTo(100, 2); // Allow for slight rounding errors
    });

    test('ensures no probability is under 0.01', () => {
        const lowProbabilityLevels = performanceTechLevelList
