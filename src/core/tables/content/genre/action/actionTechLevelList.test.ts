...
import {actionTechLevelList} from "./actionTechLevelList";

describe('Action Tech Level List', () => {
    test('tests action tech level list', () => {
        const totalProbability = actionTechLevelList.reduce((sum, level) => sum + level[1], 0);
        expect(totalProbability).toBeCloseTo(100, 0); // Allow for slight rounding errors
    });

    test('ensures no probability is under 0.01', () => {
        const lowProbabilityLevels = actionTechLevelList
